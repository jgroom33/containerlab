...
Only partial startup configuration is supported. XML files (for NETCONF) and
text-based CLI configs are both accepted, as long as the filename includes the
`.partial` suffix. XML configs are applied via NETCONF; CLI configs (for example
`.cfg.partial`, `.conf.partial`, or `.txt.partial`) are applied over SSH to the
management plane after the device reaches the `ssh_ready` state.
